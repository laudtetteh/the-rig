...
| `/rig-upgrade` | Upgrade workflow | Pulls latest Rig source; survey phase always runs `--strategy agent-plan` as a read-only JSON preview. `--mode=agent` (also the non-interactive default) runs `install.sh --strategy agent-upgrade`, presents any `conflicts[]` with repair guidance, then runs `bin/rig doctor --json` as a post-upgrade gate. `--mode=classic` runs plain `install.sh --strategy upgrade`, the original per-file interactive review flow. `--version` prints the project version, global installer version, and the latest tagged release from GitHub (via `gh`), warning if any are out of sync. `--scope=project\|global\|both` limits which layers are upgraded. |
