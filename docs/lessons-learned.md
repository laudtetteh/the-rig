...
**Root cause**: The #451 fix routed `_stealth_install_git_hook()`'s protection check through `upgrade_prepare_mutation()`, a shared helper whose very first line is `[[ "$COLLISION_STRATEGY" == upgrade ]] || return 0` — by design, since its other ~13 callers are genuinely upgrade-only operations (settings merges, `.rigpath`, `.rig/VERSION`, etc.). Git-hook installation is not upgrade-only — it needs to happen, and be protected, under every strategy, including `merge` (the default for a fresh install). Routing it through an upgrade-only guard silently no-opped the entire check for six of the seven strategy values. This was invisible to `agent-plan`/`agent-upgrade` testing specifically because both of those internally set `COLLISION_STRATEGY="upgrade"` regardless of their own flag name — so extensive testing of the agent-driven contract, including a prior holistic combinatorial review that reported zero findings, never actually exercised the vulnerable path. The regression test added alongside the original #451 fix also only covered `--strategy upgrade`, since the test file's own shared helper hardcodes that strategy.
