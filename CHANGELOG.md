...
- **Stop hook** (`templates/project/.claude/hooks/stop.sh`, wired as `"Stop"` event in `settings.json`): fires automatically when Claude Code's agent finishes its final response. Updates the `Last updated:` date in `CONTEXT_SNAPSHOT.md` (preserves the description — only the date changes) so the freshness signal stays accurate even when `/wrap` isn't run. Appends a `<!-- session-end YYYY-MM-DD HH:MM -->` boundary comment to `PROGRESS.md` — the heuristic in `/wrap` and `/post-merge` uses this to determine which entries belong to the current session. Idempotent: skips if a marker is already the last non-blank line.
